...
from 
hacker h join submissions s on h.hacker_id=s.hacker_id 
group by h.hacker_id,h.name order by total_score desc, h.hacker_id;

WITH HackerScores AS (
    SELECT h.hacker_id,h.name,SUM(s.score) AS total_score
    FROM hacker h JOIN submissions s ON h.hacker_id = s.hacker_id
    GROUP BY h.hacker_id, h.name
)
SELECT 
    hacker_id,name,total_score FROM HackerScores ORDER BY total_score DESC, hacker_id;
...
from (
    select
        ...
        max(purch_amt) as max_amount
    from odr group by ord_date
) as ordersums where total_amount >= max_amount + 1000.00;
...
with cte as(
select *,
dense_rank() OVER (/* partition by d.dpt_allotment */order by d.dpt_allotment) as dept_rank 
